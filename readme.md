...
你知道这种感觉。你做了那个调整,点击<kbd>⌘S</kbd> <kbd>⌘Tab</kbd> <kbd>⌘R</kbd>,...**没有什么变化**.你得到旧版本。你击败了捆绑器。你等了几秒钟,再次打了<kbd>⌘R</kbd>,你的变化终于出现了。但为时已晚 -**你失去了动力.**这是错误的粉红色。你用 z 拼写"菜单"。该错误有时仍会发生.

冲洗。重复。十个周期之后,事情看起来很好。现在是`git commit`时候了，但是你花在**等待的时间比工作要多得多**。这是你的捆绑器的错.

Pax 是一个捆绑器。但你永远不会打败它。为什么?

- 它是并行化的。它充分利用了您的 CPU。
- 这是最小的。它不会因您不需要的功能而陷入困境。
